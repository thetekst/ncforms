NcForms.window.UpdateNcForm = function(config) {
    config = config || {};
	config.id = Ext.id();
    Ext.applyIf(config,{
        title: ('Update form')
    ...
    NcForms.window.UpdateNcForm.superclass.constructor.call(this,config);
...
Ext.extend(NcForms.window.UpdateNcForm,MODx.Window, {
	...
	,successMsg: function (field, record, i) {
		...
		switch (field.value) {
			...
			default:
				...
				redirectId.hide();
				// requires a non-empty value
				redirectId.allowBlank = true;
		}
	}
});
Ext.reg('ncforms-window-ncform-update',NcForms.window.UpdateNcForm);